...
static const char * get_model_or_skip(int argc, char * argv[]) {
    if (argc > 1) {
        return argv[1];
    }
    const char * model_path = getenv("LLAMACPP_TEST_MODELFILE");
    if (!model_path || model_path[0] == '\0') {
        fprintf(stderr, "no model file provided, skipping; set LLAMACPP_TEST_MODELFILE or pass a GGUF path\n");
        exit(EXIT_SUCCESS);
    }
    return model_path;
}
...
int main(int argc, char * argv[]) {
    const char * model_path = get_model_or_skip(argc, argv);
